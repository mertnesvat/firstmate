...
test_teardown_conformance_old_vs_new() {
  ...
  assert_contains "$(cat "$log_old")" "tmux"$'\x1f''kill-window'$'\x1f''-t'$'\x1f'"=firstmate:=fm-$id" \
    "baseline teardown fixture did not preserve exact session and window selectors"
  ...
  pass "fm-teardown.sh: treehouse return remains compatible while baseline and current tmux cleanup use exact selectors"
}
